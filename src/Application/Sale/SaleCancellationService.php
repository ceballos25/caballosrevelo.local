...
final class SaleCancellationService
{
    ...
    public function cancelPartial(int $saleId, array $ticketIds, int $adminId, ?string $notes = null): array
    {
        ...
        if ($ticketIds === []) {
            throw new DomainException('Debe seleccionar al menos un número', 'NO_TICKETS');
        }
        ...
        foreach ($ticketIds as $tid) {
            if (!in_array($tid, $saleTicketIds, true)) {
                throw new DomainException('El número no pertenece a esta venta', 'TICKET_NOT_IN_SALE');
            }
        }
