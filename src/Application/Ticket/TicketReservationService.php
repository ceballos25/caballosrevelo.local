        ...
        try {
            ...
            if ($ticketIds !== null && $ticketIds !== []) {
                ...
                if (count($ticketIds) !== $quantity) {
                    throw new DomainException('La cantidad no coincide con los números seleccionados', 'QUANTITY_MISMATCH');
                }
                $rows = $this->tickets->findByIdsForUpdate($raffleId, $ticketIds);
                if (count($rows) !== count($ticketIds)) {
                    throw new DomainException('Algunos números no existen', 'TICKETS_NOT_FOUND');
                }
                foreach ($rows as $row) {
                    if ((int)$row['status_ticket'] !== TicketStatus::AVAILABLE) {
                        throw new DomainException(
                            'El número ' . $row['number_ticket'] . ' no está disponible',
                            'TICKET_UNAVAILABLE'
                        );
                    }
                }
                ...
                if (count($rows) < $quantity) {
                    throw new DomainException('No hay suficientes números disponibles', 'INSUFFICIENT_TICKETS');
                }
            ...
            if ($reserved !== count($ticketIds)) {
                throw new DomainException(
                    'No se pudieron reservar todos los números. Intenta de nuevo.',
                    'RESERVATION_RACE'
                );
            }
