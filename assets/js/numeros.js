...
function renderNumeroMobile(t, status, badge, premiumControl) {
    ...
    <div class="admin-card-head d-lg-none">
        <div class="admin-card-head__meta admin-card-head__meta--numero">
            ${adminNumeroInventarioBadge(t.number_ticket, status)}
        </div>
...
function renderNumeroRow(t) {
    ...
    <tr class="align-middle border-bottom card-row-admin card-row-numeros numeros-table-row">
        <td class="py-3 ps-3 mobile-card-head">
            ...
            <div class="d-none d-lg-flex numeros-col-numero-desktop">${adminNumeroInventarioBadge(t.number_ticket, status)}</div>
        </td>
...
async function cambiarEstadoTicket(id, nuevoEstado) {
    ...
    if (typeof Swal === 'undefined') {
        ...
        if (!window.confirm(`¿Deseas ${accion} este número?`)) return;
        try {
            const fd = new FormData();
    ...
    const result = await Swal.fire({
            title: esLiberar ? '¿Liberar número?' : '¿Reservar número?',
            html: numero
                ? `<p class="mb-2 text-muted">El número quedará como <strong>${esLiberar ? 'disponible' : 'reservado'}</strong>.</p>
                   ${crNumeroChipHtml(numero, 'lg')}`
                : `<p class="mb-0 text-muted">El número quedará como <strong>${esLiberar ? 'disponible' : 'reservado'}</strong>.</p>`,
            icon: 'question',
    ...
    if (numerosToast) {
        numerosToast.fire({
            icon: 'success',
            title: esLiberar ? 'Número liberado' : 'Número reservado',
        });
    }
