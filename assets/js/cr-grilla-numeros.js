(function (global) {
    ...
    function renderNumeroCell(t, selectedSet, onToggleName) {
        ...
        if (!disponible) {
            return `<span class="${classes.join(' ')}" data-ticket-id="${id}" role="img" aria-label="Número ${num}, ${statusLabel(t)}" title="${statusLabel(t)}">${num}</span>`;
        }

        const fn = onToggleName || 'CrGrillaNumeros.toggle';
        return `<button type="button" class="${classes.join(' ')}" data-ticket-id="${id}" aria-pressed="${selected ? 'true' : 'false'}" aria-label="Número ${num}, ${selected ? 'seleccionado' : 'disponible'}" onclick="${fn}(${id})">${num}</button>`;
    }
    ...
    function render(config) {
        const {
            ...
            emptyMessage = 'No hay números para esta rifa.',
        } = config;
    ...
    function patchSelection(gridId, ticketId, isSelected) {
        ...
        cell.setAttribute(
            'aria-label',
            `Número ${num}, ${isSelected ? 'seleccionado' : 'disponible'}`
        );
        return true;
    }
