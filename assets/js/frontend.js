...
async function cargarRifasActivas() {
    try {
        ...
        if (data.success) estado.rifasActivas = data.data || [];
            ...
            toastError(data.message || 'No se pudieron cargar las rifas activas.');
        }
    } catch (e) {
        if (typeof toastError === 'function') {
            toastError('No se pudieron cargar las rifas. Recarga la página.');
        }
    }
}
...
async function cargarInventario() {
    ...
    try {
        ...
        try {
            ...
            throw new Error('Respuesta inválida al cargar números disponibles.');
        }
        ...
        if (json.success) {
            ...
            toastError(json.message || 'No se pudo cargar el inventario de números.');
        }
    } catch (e) {
        toastError(e instanceof Error ? e.message : 'Error al cargar números disponibles.');
    }
...
function renderSelectorManualNumeros() {
    ...
    CrGrillaNumeros.render({
        ...
        emptyMessage: 'No hay números para mostrar.',
    });
}
...
function toggleNumeroManual(idTicket) {
    ...
    if (ticket && typeof CrGrillaNumeros !== 'undefined' && !CrGrillaNumeros.isDisponible(ticket)) {
        toastError('Este número ya no está disponible.');
        return;
    }
...
function validarSeleccionManualCheckout() {
    ...
    if (estado.numerosSeleccionados.length < min) {
        toastError(`Selecciona al menos ${min} número${min > 1 ? 's' : ''} en la grilla`);
        return false;
    }
    estado.cantidadSeleccionada = estado.numerosSeleccionados.length;
    return true;
...
$('#cantidadManual').on('blur', function () {
    ...
    if (cant < min) {
        ...
        toastError(`Recuerda mínimo ${min} sticker${min > 1 ? 's' : ''} para participar`);
...
function pricingTierConfig() {
    ...
    return {
        enabled: true,
        firstUnit: readPricingInt(p?.first_unit ?? setting('pricing_first_unit', 65000), 65000),
        secondUnit: readPricingInt(
            p?.second_unit ?? p?.tier1_unit ?? setting('pricing_tier1_unit', 60000),
            60000
        ),
        thirdPlusUnit: readPricingInt(
            p?.third_plus_unit ?? p?.tier2_unit ?? setting('pricing_tier2_unit', 55000),
            55000
        ),
    };
}
...
function mensajePromoPrecio(cantidad, variant = 'full') {
    ...
    if (variant === 'hero') {
        ...
        if (qty === 2) {
            return `Promo 2 nums · ${fmtCOPShort(tier.secondUnit)} c/u · ${fmtCOPShort(total2)}`;
        }
        if (qty === 1) {
            return `2 números: ${fmtCOPShort(tier.secondUnit)} c/u · ${fmtCOPShort(total2)}`;
        }
        return '';
    }
    ...
    if (qty === 2) {
        return `PROMO 2 NÚMEROS — ${fmtCOPShort(tier.secondUnit)} c/u (${fmtCOPShort(total2)} total)`;
    }

    if (qty === 1) {
        return `2 números: ${fmtCOPShort(tier.secondUnit)} c/u cada uno`;
    }
...
function crearChipSeleccionadoHtml(id, num) {
    return `<button type="button" class="cr-chip-seleccionado" data-ticket-id="${id}" onclick="toggleNumeroManual(${id})" title="Quitar número ${num}">
...
function renderSeleccionadosBarSmooth(container) {
    ...
    if (!esRifaManual() || !estado.numerosSeleccionados.length) {
        container.innerHTML = `<div class="cr-seleccionados-pack">
            <span class="cr-seleccionados-pack__badge">${cant} sticker${cant > 1 ? 's' : ''}</span>
            <span class="cr-seleccionados-pack__hint">Números al confirmar el pago</span>
        </div>`;
...
function actualizarCheckoutSheet() {
    ...
    if (sheetCant) {
        sheetCant.textContent = `${cant} número${cant !== 1 ? 's' : ''}`;
    }
...
function actualizarUI() {
    ...
    $('#cantTicketsDesktop').text(cant);

    const mobileQty = document.getElementById('lblCantidadMobileCart');
    if (mobileQty) {
        mobileQty.textContent = `${cant} num${cant !== 1 ? 's' : ''}`;
    }
    ...
    $('#resumenNumeros').html(
        cant
            ? `<span class="fw-bold">${cant} número${cant > 1 ? 's' : ''}</span>`
...
async function abrirCheckout() {
    ...
    if (esRifaManual()) {
        if (estado.numerosSeleccionados.length < min) {
            toastError(`Selecciona al menos ${min} número${min > 1 ? 's' : ''}`);
            return;
        }
        estado.cantidadSeleccionada = estado.numerosSeleccionados.length;
    } else if (!estado.cantidadSeleccionada || estado.cantidadSeleccionada < min) {
        toastError(`La compra mínima es de ${min} número${min > 1 ? 's' : ''}`);
        return;
    }
...
async function procesarTransferencia(e) {
    ...
    if (estado.cantidadSeleccionada < minCantidadCompra()) {
        toastError(`Mínimo ${minCantidadCompra()} número${minCantidadCompra() > 1 ? 's' : ''}`);
        return;
    }
